...
async function handleLargeCompressedFile(
  ...
  if (!provider) {
    vscode.window.showErrorMessage(
      `No provider found for file: ${path.basename(filePath)}`,
    );
    return false;
  }
  ...
  try {
    ...
    const handled = await provider.handleLargeFile(
      filePath,
      decompressedData,
      MAX_PREVIEW_BYTES,
      currentTabUri,
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.window.registerCustomEditorProvider(
      'compressed-log-viewer.compressed',
      {
        ...
        async resolveCustomEditor(
          ...
          try {
            ...
            vscode.window.showErrorMessage(
              `Failed to open compressed file: ${err}`,
            );
            await closeTabByUri(
              document.uri,
              'compressed-log-viewer.compressed',
